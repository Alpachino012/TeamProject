...
from src.services.live_sources import build_live_functions
...
def build_researcher(settings: Settings, *, offline: bool = False) -> Researcher:
    functions = build_offline_functions() if offline else build_live_functions(settings)
    ai_service = AIService(settings, functions=functions)
    cache = JsonFileSourceCache(
    settings.cache_dir / ("sources-offline.json" if offline else "sources.json"),
